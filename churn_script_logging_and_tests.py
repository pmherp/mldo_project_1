...
def test_import(import_data):
	...
	try:
		...
		logging.error("ERROR testing import_data: The file doesn't appear to have rows and columns. Input type not string. Filepath not found.")
		raise err
...
def test_generate_churn_column(generate_churn_column, df):
	...
	try:
		...
		logging.error('ERROR testing generate_churn_column: Attrition_Flag column not in input data.')
		raise err	
...
def test_generate_hist_plot(generate_hist_plot, df):
	...
	try:
		...
		logging.error('ERROR testing generate_hist_plot: Columns "Churn" and "Customer_Age" not in input data. Path to save plots does not exist.')
		raise err
...
def test_split_cat_quant_cols(split_cat_quant_cols, df):
	...
	try:
		...
		logging.info('ERROR testing split_cat_quant_cols function: columns not in input data.')
		raise err
...
def test_encoder_helper(encoder_helper, category_lst, df):
	...
	try:
		...
		logging.error('ERROR testing encoder_helper function: category list is empty.')
		raise err
...
def test_perform_feature_engineering(perform_feature_engineering, df):
	...
	try:
		...
		logging.error('ERROR testing perform_feature_engineering function: input of train, test splits empty.')
		raise err
...
def test_train_models(train_models, X_train, X_test, y_train, y_test):
	...
	try:
		...
		logging.info('SUCCESS: testing train_models function for dump rfc_model.')
	except AssertionError as err:
		logging.error('ERROR testing train_model function: path to store rfc_model does not exist.')
		raise err
	
	try:
		#check if path to save models exists
		assert os.path.exists('./models/logistic_model.pkl')
		logging.info('SUCCESS: testing train_models function for dump logistic_model.')
	except AssertionError as err:
		logging.error('ERROR testing train_model function: path to store logistic_model does not exist.')
		raise err
...
if __name__ == "__main__":
	...
	X_train, X_test, y_train, y_test = test_perform_feature_engineering(cls.perform_feature_engineering, df)
